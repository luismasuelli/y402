...
app.get("/health", (_req, res) => {
    res.json({ status: "healthy", service: "dumb-x402-facilitator-express" });
});
...
const port = Number(process.env.FACILITATOR_PORT || "9876");
app.listen(port, () => {
    console.log(`[express-facilitator] listening on http://localhost:${port}`);
});
